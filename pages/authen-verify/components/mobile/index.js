    ...
    {/* <UseEFOTP/> */}
      <Box sx={{background:`linear-gradient(${themedata[0].primary}, ${themedata[0].three})`,height:"100vh",width:'100%'}}>
        ...
        transform: 'translate(-50%, -50%)'}}> 
          ...
          wordWrap: 'break-word',}}>{TOTPText[0].title}</Box>
          <Box pb={3} pt={2} sx={{color: `${themedata[0].four}`, fontSize: 15, fontFamily: frontdata[0].font, fontWeight: '0', 
          textAlign: 'center'}}>{TOTPText[0].description}</Box>
          <Box pb={3} >
          <OtpInput
          className='otp-input-container'
          value={state.totp}
        onChange={(newdata)=>{setstate((prevData) => ({ ...prevData, totp: newdata}));}}
          numInputs={6}
