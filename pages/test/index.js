...
const handleClose = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://192.168.5.46:8008/register-chiccrm", {
        method: 'POST',
      ...
      <Box p={3}>
          ...
          <Grid container  pl={5}  columnSpacing={{ xs: 1, sm: 2, md: 3 }} sx={{  width: '50%' }}>
           ...
           <InputLabel id="country-select-label">Country</InputLabel>
            ...
            <Grid item xs={4}>
            <TextField fullWidth  label="Postal Code" variant="outlined" size='small'   value={state.zipcode}  InputProps={{ readOnly: true, }} style={{ width: '300px', height: '60px' }} focused />
            </Grid>     
            <Grid item xs={4}pb={2}>
