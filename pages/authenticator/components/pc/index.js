...
import { totp } from '@/data/metadata';
...
function Index() {
  ...
  return (
   <>
        ...
        <Box sx={{ color: `${themedata[0].ten}`, fontSize: 20, fontFamily: frontdata[0].font, fontWeight: '800', wordWrap: 'break-word',}}>{totp[0].title}</Box>
        <Box pt={2} sx={{color: `${themedata[0].four}`, fontSize: 15, fontFamily: frontdata[0].font, fontWeight: '0', textAlign: 'center'}} dangerouslySetInnerHTML={{ __html: totp[0].description }}/>
        <Box pb={3} pt={1} sx={{color: "#2AB930", fontSize: 13, fontFamily: frontdata[0].font, fontWeight: '0', textAlign: 'center'}} dangerouslySetInnerHTML={{ __html: totp[0].description2 }}/>
        <Box className="borderotp" >
          <Box sx={{mt:2.5,mb:2.5,ml:2.5,mr:2.5}}>
             <QRCode value={state.qrcode_url} size={150} level={"H"} />
          </Box>
        </Box>
        <Box pt={2} sx={{cursor:"pointer",color: `${themedata[0].primary}`, fontSize: 15, fontFamily: frontdata[0].font, fontWeight: '0', textAlign: 'center'}}>{totp[0].cantscan}</Box>
        <Box pb={1} pt={5} sx={{color: `${themedata[0].four}`, fontSize: 15, fontFamily: frontdata[0].font, fontWeight: '0', textAlign: 'center'}}>{totp[0].typethecode}</Box>
        <Box pb={3} pt={2} >
        ...
        <Button disabled={state.loading?true:false} onClick={handleclick}  variant='contained'  style={{ fontSize: '12px', padding: '12px 16px',backgroundColor:`${themedata[0].primary}`,
